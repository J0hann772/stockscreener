...
Сервисы общаются между собой по HTTP внутри сети `internal`. Redis используется исключительно как кэш, в будущем может быть использован как транспорт между сервисами.
